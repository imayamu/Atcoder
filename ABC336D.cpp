...
int findMaxPyramidSize(const vector<int>& A) {
    ...
    for (int i = 0; i < N; ++i) {
        if (i == 0) left[i] = 1;
        else left[i] = min(left[i - 1] + 1, A[i]);
    }

    
    for (int i = N - 1; i >= 0; --i) {
        if (i == N - 1) right[i] = 1;
        else right[i] = min(right[i + 1] + 1, A[i]);
    }

    
    int maxPyramidSize = 0;
    for (int i = 0; i < N; ++i) {
        int currentSize = min(left[i], right[i]);
        maxPyramidSize = max(maxPyramidSize, currentSize);
    }
...
int main() {
    ...
    cout << findMaxPyramidSize(A) << endl;
